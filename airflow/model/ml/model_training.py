...
def define_X_Y_variables(apartments_for_rent):
    ...
    poi_types = ["food", "attractions", "shopping", "grocery"]
    poi_features = [
        f"{p}_nearest_dist" for p in poi_types
    ] + [
        f"{p}_count_200m" for p in poi_types
    ]
    base_cols = base_cols + [col for col in poi_features if col in apartments_for_rent.columns]
...
def compute_fair_rent_oof(
    ...
    """
    FULL TRUE OOF FAIR RENT PREDICTION (random K-fold)
    --------------------------------------------------
    This produces:
      - fair_rent (OOF predictions)
      - mispricing (actual - fair)
      - pct_mispricing

    Critically:
      - No leakage from KNN lags
      - H3 target encoding uses train only
      - Every prediction comes from a model that never saw the point
    """
    ...
    cols_with_coords = x_cols + ["latitude", "longitude"]

    for fold_id, (train_pos, test_pos) in enumerate(kf.split(apartments_for_rent)):
        print(f"Processing fold {fold_id}...")

        train_idx = apartments_for_rent.index[train_pos]
        test_idx  = apartments_for_rent.index[test_pos]
        ...
        y_train = y_array[train_pos]
        y_test  = y_array[test_pos]

        y_train_series = pd.Series(y_train, index=train_idx)
        global_mean = y_train_series.mean()
        ...
        train_resid = y_train - baseline.predict(X_train_base)
    ...
    apartments_for_rent_out["log_fair_rent"] = fair_pred

    apartments_for_rent_out["actual_rent"] = np.exp(apartments_for_rent_out["log_rent"])
    apartments_for_rent_out["fair_rent"] = np.exp(apartments_for_rent_out["log_fair_rent"])

    apartments_for_rent_out["pct_mispricing"] = (
        (apartments_for_rent_out["actual_rent"] - apartments_for_rent_out["fair_rent"])
        / apartments_for_rent_out["fair_rent"]
    )

    apartments_for_rent_out["mispricing"] = apartments_for_rent_out["actual_rent"] - apartments_for_rent_out["fair_rent"]
    apartments_for_rent_out["mispricing_pct"] = apartments_for_rent_out["mispricing"] / apartments_for_rent_out["actual_rent"]
